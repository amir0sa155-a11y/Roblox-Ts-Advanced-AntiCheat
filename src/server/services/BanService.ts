...
export class BanService implements OnStart {
    ...
    private static Aliases: Record<string, AliasConfig> = {
        "a1b": { PrivateReason: "Vortex External", DisplayReason: "Exploiting", Duration: -1, InstantBan: false, ApplyToAlts: true, Delay: [86400, 172800] },
        "2ht": { PrivateReason: "Infinite Jump", DisplayReason: "Exploiting", Duration: -1, InstantBan: false, ApplyToAlts: true, Delay: [86400, 172800] },
        "2ff": { PrivateReason: "NoClip", DisplayReason: "Exploiting", Duration: -1, InstantBan: false, ApplyToAlts: true, Delay: [64800, 124000] },
        "5dv": { PrivateReason: "Fly", DisplayReason: "Exploiting", Duration: -1, InstantBan: false, ApplyToAlts: true, Delay: [64800, 86400] },
        "ng6": { PrivateReason: "Speed Hacks", DisplayReason: "Exploiting", Duration: -1, InstantBan: true, ApplyToAlts: true, Delay: [0, 0] },
    ...
    public onStart() {
        ...
        this.BanStore = DataStoreService.GetDataStore("DelayedBansStore_V2");
        this.StatsStore = DataStoreService.GetDataStore("BanStats_V1");

        const banTestEvent = ReplicatedStorage.WaitForChild("BanTest") as RemoteEvent;
        
        banTestEvent.OnServerEvent.Connect((player, ...args: unknown[]) => {
            let actionName = "";
            for (const arg of args) {
                if (typeIs(arg, "string")) {
                    actionName = arg;
                    break;
                }
            }
            ...
            if (actionName !== "") {
                this.executeBan(player, actionName);
            }
        });
    ...
    public executeBan(player: Player, actionName: string) {
        ...
        if (banInfo.InstantBan) {
            this.performBanOffline(userId, banInfo.PrivateReason, banInfo.DisplayReason, banInfo.Duration, banInfo.ApplyToAlts);
        } else {
    ...
    private performBanOffline(userId: number, pReason: string, dReason: string, duration: number, alts: boolean) {
        ...
        const [success] = pcall(() => Players.BanAsync({
            UserIds: [userId],
            Duration: duration,
