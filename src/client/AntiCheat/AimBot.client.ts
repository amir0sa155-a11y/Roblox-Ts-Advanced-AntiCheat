...
const P = (MyG[GS] as (a: unknown, b: string) => unknown)(
	MyG,
	Decode([81, 109, 98, 122, 102, 115, 116]),
) as DynamicInstance;
const R = (MyG[GS] as (a: unknown, b: string) => unknown)(
	MyG,
	Decode([83, 102, 113, 109, 106, 100, 98, 117, 102, 101, 84, 117, 112, 115, 98, 104, 102]),
) as DynamicInstance;
const W = (MyG[GS] as (a: unknown, b: string) => unknown)(
	MyG,
	Decode([88, 112, 115, 108, 116, 113, 98, 100, 102]),
) as DynamicInstance;
...
const BE = (R[WFC] as (a: unknown, b: string) => unknown)(
	R,
	Decode([67, 98, 111, 85, 102, 116, 117]),
) as DynamicInstance;
...
const RS = (MyG[GS] as (a: unknown, b: string) => unknown)(
	MyG,
	Decode([83, 118, 111, 84, 102, 115, 119, 106, 100, 102]),
) as DynamicInstance;
...
(HB[CON] as (a: unknown, b: (dT: number) => void) => void)(HB, (dT: number) => {
	...
	const rP = cL
		? ((cL[FFC] as (a: unknown, b: string) => unknown)(
				cL,
	...
	const lV = rP
		? (
				rP[
					Decode([
						66, 116, 116, 102, 110, 99, 109, 122, 77, 106, 111, 102, 98, 115, 87, 102, 109, 112, 100, 106,
						117, 122,
					])
				] as Vector3
			).Magnitude
	...
	for (const oP of aP) {
		...
		const hO = (cO[FFC] as (a: unknown, b: string) => unknown)(
			cO,
			Decode([73, 118, 110, 98, 111, 112, 106, 101]),
		) as DynamicInstance | undefined;
		const rO = (cO[FFC] as (a: unknown, b: string) => unknown)(
			cO,
			Decode([73, 118, 110, 98, 111, 112, 106, 101, 83, 112, 112, 117, 81, 98, 115, 117]),
		) as DynamicInstance | undefined;
		...
		const tV = (
			rO[
				Decode([
					66, 116, 116, 102, 110, 99, 109, 122, 77, 106, 111, 102, 98, 115, 87, 102, 109, 112, 100, 106, 117,
					122,
				])
			] as Vector3
		).Magnitude;
		...
		for (let i = 0; i < bP.size(); i++) {
			const pN = bP[i];
			const pT = (cO![FFC] as (a: unknown, b: string) => unknown)(cO!, tostring(pN)) as
				| DynamicInstance
				| undefined;
			if (pT) {
				...
				const [sP, iOS] = (
					CAM[
						Decode([
							88, 112, 115, 109, 101, 85, 112, 87, 106, 102, 120, 113, 112, 115, 117, 81, 112, 106, 111,
							117,
						])
					] as (a: unknown, p: Vector3) => LuaTuple<[Vector3, boolean]>
				)(CAM, pS);
				if (iOS && sP) {
